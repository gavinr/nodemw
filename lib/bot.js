...
module.exports = (function () {
  ...
  return page.revisions
      ...
      this.getToken(title, 'edit', function (err, token) {
        ...
        self.api.call(editParams, function (err, data) {
          if (!err && data.result && data.result === 'Success') {
            ...
            callback(null, data)
          } else if (!err && data.result && data.result === 'Failure' && data.captcha) {
            // we need to answer the captcha first.
            if (data.captcha.type === 'question' && data.captcha.mime === 'text/plain') {
              if (params.captchaAnswers.hasOwnProperty(data.captcha.question)) {
                var answer = params.captchaAnswers[data.captcha.question]
                // call the edit again, this time with the captcha id and answer!
                self.doEdit(type, title, summary, _.extend({
                  captchaid: data.captcha.id,
                  captchaword: answer
                }, params), callback)
          } else {
                callback(new Error(`You need to provide an answer to the captcha: ${JSON.stringify(data.captcha)}`))
              }
            } else {
              callback(new Error(`TODO: Need to support other captcha types. Edit failed: ${err}`))
            }
          } else {
    ...
    edit (title, content, summary, captchaAnswers, minor, callback) {
      let params = {
        text: content
      }
      ...
      if (captchaAnswers) {
        params.captchaAnswers = captchaAnswers
      } else {
        params.captchaAnswers = false
      }
